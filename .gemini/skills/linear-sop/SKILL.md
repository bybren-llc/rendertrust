...
```bash
# Via Linear Web UI
# Navigate to: https://linear.app/team/{{PROJECT_TEAM_NAME}}/issue/REN-XXX

# Or use Linear CLI if installed
linear issue view REN-XXX
```
...
# Or use Linear CLI:
linear issue update REN-XXX --state "Done"
```
...
# Or use Linear CLI:
linear issue comment REN-XXX "**Dev Evidence**\n\n..."
```
...
Tickets referenced in commit messages (e.g., `[REN-123]`) automatically move to **Done** when the PR merges. Child stories not referenced in any commit message must be manually closed after merge.
...
- Branch name contains `REN-XXX`
- PR title contains `[REN-XXX]`
