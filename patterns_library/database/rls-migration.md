...
-- 5. Create RLS policy for superuser (rendertrust)
...
-- 6. Create RLS policy for application user (bybren_app_user)
...
DATABASE_URL="postgresql://rendertrust:rendertrust_dev@localhost:5432/rendertrust_dev" \
npx prisma migrate dev --name add_user_preferences_with_rls
...
- 2025-10-03: Created table with RLS policies [REN-300]
```
...
# Test user isolation
psql -U bybren_app_user -d rendertrust_dev \
