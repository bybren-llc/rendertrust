...
```
{{GITHUB_ORG}}/
├── RenderTrust/             # Main application repo (PRIMARY)
│   ├── .claude/                  # Full harness
│   ├── dark-factory/             # Agent teams run here
│   └── ...
│
└── RenderTrust-packages/    # Shared packages repo (SECONDARY)
...
cp -r ../RenderTrust/.claude/ .claude/
cp -r ../RenderTrust/.gemini/ .gemini/
cp ../RenderTrust/CLAUDE.md .
cp ../RenderTrust/AGENTS.md .

# Adjust project-specific values
sed -i "s/RenderTrust/RenderTrust-packages/g" CLAUDE.md
```
...
2. **Multiple sessions** — Run separate dark factory sessions for each repo:
   ...
   # Session 1: workspace
   cd RenderTrust
   ./dark-factory/scripts/factory-start.sh feature REN-10
...
git clone git@github.com:{{GITHUB_ORG}}/RenderTrust.git
cd RenderTrust
...
```bash
FACTORY_PROJECT_DIR="/home/{{REMOTE_USER}}/RenderTrust"
FACTORY_MAIN_BRANCH="{{MAIN_BRANCH}}"
FACTORY_TICKET_PREFIX="REN"
...
cd /home/{{REMOTE_USER}}
git clone git@github.com:{{GITHUB_ORG}}/RenderTrust-packages.git

# Start a separate factory session pointing to packages
cd RenderTrust-packages
# Temporarily override FACTORY_PROJECT_DIR
FACTORY_PROJECT_DIR="$(pwd)" ./dark-factory/scripts/factory-start.sh story REN-11
```
