...
```
I need to implement REN-42 (User Profile Management).
The spec is at specs/REN-42-user-profile-spec.md.
...
```
I need a thorough review of the changes on branch REN-99-payment-flow.
...
- For Stories where the implementer can handle the PR (RTE role collapsed per REN-499), a team of 2 (implementer + QAS) is often sufficient.
...
if [ "$TEAMMATE_ROLE" = "qas" ]; then
  ...
  if [ ! -f "test-results/REN-latest.json" ]; then
...
./dark-factory/scripts/factory-start.sh feature REN-42
