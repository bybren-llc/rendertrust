...
git checkout REN-XXX-your-feature
...
mcp__claude_ai_Linear__search_issues "assignee:me state:Ready"
mcp__claude_ai_Linear__search_issues "assignee:me state:In Progress"
```
...
grep -A 30 "Low-Level Tasks" specs/REN-XXX-{feature}-spec.md

# Check for security-critical requirements
grep "#EXPORT_CRITICAL" specs/REN-XXX-{feature}-spec.md

# Check for architectural decisions that affect your work
grep "#PATH_DECISION" specs/REN-XXX-{feature}-spec.md
```
...
3. **Branch is created** (if one does not exist):
   ...
   ```bash
   git checkout -b REN-XXX-short-description
   ```
...
# Should show: REN-XXX-short-description

# Confirm spec is available
ls specs/REN-XXX-*-spec.md
```
...
```
@system-architect I need to implement [feature] for REN-XXX.
...
git commit -m "feat(api): add user profile endpoint with RLS context [REN-123]"

# Bug fix
git commit -m "fix(ui): resolve login redirect loop on expired session [REN-456]"

# Test addition
git commit -m "test(api): add integration tests for profile endpoint [REN-123]"

# Multi-line commit for complex changes
git commit -m "$(cat <<'EOF'
feat(payments): add Stripe checkout flow with webhook handling [REN-789]
...
- [ ] **Commit messages follow format**: `type(scope): description [REN-XXX]`
...
git push --force-with-lease origin REN-XXX-your-feature
