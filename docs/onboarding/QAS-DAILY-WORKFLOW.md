...
- Access to Linear workspace with `mcp__claude_ai_Linear__*` tools available
...
mcp__claude_ai_Linear__search_issues "assignee:me state:Testing"

# Alternatively, review tickets moved to Testing since last session
mcp__claude_ai_Linear__search_issues "state:Testing updated:>yesterday"
```
...
grep -A 20 "Acceptance Criteria" specs/REN-XXX-{feature}-spec.md
...
```markdown
## QAS Validation Report - REN-XXX

**Session ID**: [Claude Code session ID]
**Date**: [YYYY-MM-DD]
**Spec**: specs/REN-XXX-{feature}-spec.md
...
QAS uses `mcp__claude_ai_Linear__*` tools throughout the workflow. Here are the key interactions:

### Reading Ticket Context

```bash
# Get full ticket details
mcp__claude_ai_Linear__get_issue "REN-XXX"

# Read comments from implementers
mcp__claude_ai_Linear__get_issue_comments "REN-XXX"

# Check ticket history for context
mcp__claude_ai_Linear__get_issue_history "REN-XXX"
```
...
- **Action**: `ls specs/ | grep "REN-XXX"` or ask `@bsa`
