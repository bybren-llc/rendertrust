...
# Extract Linear ticket from commit message (REN-XXX format)
LINEAR_TICKET=$(echo "$COMMIT_MSG" | grep -oE "REN-[0-9]+" | head -1)
...
echo "   mcp__claude_ai_Linear__create_comment(issueId='$LINEAR_TICKET', body='$COMMENT')"
