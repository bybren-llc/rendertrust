...
**NEW (REN-314): RLS & Compliance Owner**
...
cat scripts/rls-phase4-final-validation.sql | docker exec -i bybren-team-postgres-1 psql -U bybren_app_user -d rendertrust_dev && npm audit --audit-level=high && yarn lint && echo "SECURITY SUCCESS" || echo "SECURITY FAILED"
```
...
grep -A 5 "Security:" specs/REN-XXX-{feature}-spec.md
```
...
```bash
# Automated RLS check
cat scripts/rls-phase4-final-validation.sql | docker exec -i bybren-team-postgres-1 psql -U bybren_app_user -d rendertrust_dev
