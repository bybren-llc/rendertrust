...
- [Agent Workflow SOP v1.4](./docs/sop/AGENT_WORKFLOW_SOP.md) - vNext contract, Exit States, Role Collapsing (REN-497/499)
...
- [Harness Whitepaper](./docs/whitepapers/CLAUDE-CODE-HARNESS-MODERNIZATION-REN-444.md) - Complete harness architecture
- [Agent Perspective](./docs/whitepapers/CLAUDE-CODE-HARNESS-AGENT-PERSPECTIVE.md) - Why the harness works
- [SAFe Methodology](https://github.com/ByBren-LLC/rendertrust) - This repository
...
| `frontend-patterns`      | UI work                | React 18, Electron, ShadCN patterns         |
...
```bash
cd sdk && ruff check . && echo "FE SUCCESS" || echo "FE FAILED"
```

### Backend Development

```bash
pytest tests/integration && echo "BE SUCCESS" || echo "BE FAILED"
```

### Pre-Push Validation

```bash
make ci && echo "CI SUCCESS" || echo "CI FAILED"
```

### Database Migration

```bash
alembic revision --autogenerate -m "migration_name" && alembic upgrade head && echo "MIGRATION SUCCESS" || echo "MIGRATION FAILED"
```
...
@rte Create PR for REN-123 and run CI validation
```
...
1. **Always reference specs**: `Read spec at specs/REN-XXX-spec.md`
...
5. **Update Linear**: TDM can update Linear with `mcp__claude_ai_Linear__create_comment`
