...
This directory contains the REN Claude Code harness: hooks, slash commands, and (coming soon) skills for workflow automation.

## Harness Architecture

```text
┌──────────────────────────────────────────────────────────────────────┐
│                      REN Claude Code Harness                         │
├──────────────────────────────────────────────────────────────────────┤
│                                                                       │
...
- **REN Round Table**: humans + AI agents are peers; Stop-the-Line authority is encouraged

Canonical reference: `.cursor/rules/06-team-culture.mdc`

## Role Execution Modes (REN-499)
...
| ---------------------- | ------------------ | ---------------------- |
| `/check-docker-status` | `/remote-status`   | Deprecated per REN-445 |
| `/deploy-dev`          | `/remote-deploy`   | Deprecated per REN-445 |
| `/dev-health`          | `/remote-health`   | Deprecated per REN-445 |
| `/dev-logs`            | `/remote-logs`     | Deprecated per REN-445 |
| `/rollback-dev`        | `/remote-rollback` | Deprecated per REN-445 |
...
## Dual-Mode Deployment (REN-445 Terminology Contract)
...
# git add . && git commit -m "feat(scope): description [REN-347]"
...
# git push --force-with-lease origin REN-347-branch
# # Use the PR template as your PR body baseline:
# # gh pr create --title "feat(scope): description [REN-347]" --body "$(cat .github/pull_request_template.md)"
# gh pr create ...
...
| Trigger               | Behavior                                                                      |
| --------------------- | ----------------------------------------------------------------------------- |
| On prompt submit      | Reminds about `REN-{number}` branch naming convention                         |
...
**Maintained by**: REN Development Team + ARCHitect-in-the-IDE (Auggie)
