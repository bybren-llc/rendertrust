    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "git branch --show-current 2>/dev/null | grep -q '^REN-[0-9]' || echo '⚠️ REMINDER: Branch should follow REN-{number}-{description} format.'",
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo '📝 REMINDER: Commit message must follow SAFe format: type(scope): description [REN-XXX]'",
...
**Note**: Customize the ticket prefix (`REN-`) and branch names (`dev`/`master`) for your project.
