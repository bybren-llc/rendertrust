...
```bash
# REN-445: Updated container name per Terminology Contract
...
```bash
ssh -i {{SSH_KEY_PATH}} {{REMOTE_USER}}@{{REMOTE_HOST}} "docker images {{REGISTRY}}/rendertrust/dev --format 'table {{.Tag}}\t{{.ID}}\t{{.CreatedAt}}'"
```
...
```bash
ssh -i {{SSH_KEY_PATH}} {{REMOTE_USER}}@{{REMOTE_HOST}} "docker pull {{REGISTRY}}/rendertrust/dev:dev-{target-sha}"
```
...
```bash
# From: image: {{REGISTRY}}/rendertrust/dev:latest
# To:   image: {{REGISTRY}}/rendertrust/dev:dev-{target-sha}
```

Use SSH + sed for inline replacement:

```bash
ssh -i {{SSH_KEY_PATH}} {{REMOTE_USER}}@{{REMOTE_HOST}} "cd {{PROJECT_PATH}} && sed -i 's|{{REGISTRY}}/rendertrust/dev:latest|{{REGISTRY}}/rendertrust/dev:dev-{target-sha}|g' docker-compose.dev.yml"
```
...
Version:  3a49b85 - feat(ci): add Slack notifications [REN-350]
...
Selected: e9722d4 - style(docs): apply markdown linting fixes [REN-347]
...
Running:  e9722d4 - style(docs): apply markdown linting fixes [REN-347]
Status:   ✅ Healthy
...
ssh -i {{SSH_KEY_PATH}} {{REMOTE_USER}}@{{REMOTE_HOST}} "cd {{PROJECT_PATH}} && sed -i 's|{{REGISTRY}}/rendertrust/dev:dev-[a-f0-9]*|{{REGISTRY}}/rendertrust/dev:latest|g' docker-compose.dev.yml"
...
- Check available tags: `docker images | grep rendertrust`
