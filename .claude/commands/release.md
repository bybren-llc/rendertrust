...
> **📋 TEMPLATE**: This command uses `dev`, `REN`, and `ByBren-LLC/{{GITHUB_REPO}}` placeholders. Replace with your project values.
...
gh pr merge <NUMBER> --squash --subject "type(scope): description [REN-XXX]"
```
...
gh api repos/ByBren-LLC/{{GITHUB_REPO}}/branches --jq '.[].name' | grep -v '^main$\|^dev$\|^template$'
```
