...
docker ps --filter name=rendertrust-staging-app --format '{{.Names}}\t{{.Status}}'

# Check for dev container (REN-400: updated container name)
docker ps --filter name=rendertrust-dev-app --format '{{.Names}}\t{{.Status}}'
```

Get current commit SHA:

```bash
# Try staging first, then dev (REN-400: updated container names)
docker inspect rendertrust-staging-app 2>/dev/null | grep 'org.opencontainers.image.revision' | cut -d'"' -f4 || \
docker inspect rendertrust-dev-app 2>/dev/null | grep 'org.opencontainers.image.revision' | cut -d'"' -f4
```
...
```bash
# For dev mode (STANDARD port 3000, REN-401)
docker ps --filter name=rendertrust-dev --format 'table {{.Names}}\t{{.Status}}'

# For staging mode (port 3001)
docker ps --filter name=rendertrust-staging --format 'table {{.Names}}\t{{.Status}}'
```
...
```bash
# Staging
docker inspect rendertrust-staging-app | grep 'org.opencontainers.image.revision' | cut -d'"' -f4

# Dev (REN-400: updated container name)
docker inspect rendertrust-dev-app | grep 'org.opencontainers.image.revision' | cut -d'"' -f4
```
...
```bash
# Staging (REN-400: use container name)
docker logs rendertrust-staging-app --tail 50

# Dev (REN-400: use container name or script)
docker logs rendertrust-dev-app --tail 50
# Or use script
...
Current:  e9722d4 - style(docs): apply markdown linting fixes [REN-347]
Status:   Up 7 hours (healthy)

Target:   3a49b85 - feat(ci): add Slack notifications [REN-350]
...
Running:  3a49b85 - feat(ci): add Slack notifications [REN-350]
Status:   Healthy
...
## Running Both Modes Simultaneously (REN-400/REN-401)
...
```bash
docker ps --filter name=rendertrust --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'
```
