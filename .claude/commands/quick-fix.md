...
- Fetch ticket: `mcp__claude_ai_Linear__get_issue $1`
- Verify it's a bug fix
- Create branch: `git checkout -b REN-$1-fix-{description}`
...
```bash
git add .
git commit -m "fix(scope): resolve {issue} [REN-XXX]"
```
