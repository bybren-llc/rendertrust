...
Use mcp__claude_ai_Linear__create_comment with:
- issueId: REN-{number}
...
**QA Reports**: `/docs/agent-outputs/qa-validations/REN-{number}-qa-validation.md`

**Naming Convention**: `REN-{number}-qa-validation.md`
...
1. **Read spec** → `cat specs/REN-XXX-{feature}-spec.md`
...
## Pattern Execution Workflow (REN-300)
...
grep -A 10 "Testing Strategy" specs/REN-XXX-{feature}-spec.md
...
3. **Linear Evidence Posted**
   - [ ] QA report created at `/docs/agent-outputs/qa-validations/REN-{number}-qa-validation.md`
   - [ ] Final verdict posted to Linear comments via `mcp__claude_ai_Linear__create_comment`

4. **Handoff Statement**
   > "QAS validation complete for REN-XXX. All criteria PASSED. Evidence posted to Linear. Approved for RTE."

**Or if BLOCKED:**

> "QAS validation BLOCKED for REN-XXX. Issues: [list]. Returning to [implementer/role] for fixes."
