...
tools: [Read, Write, Edit, Bash, Grep, Glob, mcp__claude_ai_Linear__*]
model: opus
---
...
### 0. Check Pattern Library FIRST (MANDATORY - REN-300)
...
```bash
# Create spec file for REN-XXX
cp specs/spec_template.md specs/REN-XXX-{description}-spec.md
```

#### Step 2: Extract from User Story

**From Linear ticket (REN-XXX)**:
...
ls specs/REN-*-spec.md | grep "similar_feature"
...
- Implement [feature] as specified in REN-XXX
...
- How it fits into existing rendertrust architecture
- Components affected
...
- Internal dependencies (rendertrust components)
...
From spec, add subtasks to REN-XXX:
