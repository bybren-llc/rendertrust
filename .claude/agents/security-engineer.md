...
cat scripts/rls-phase4-final-validation.sql | docker exec -i rendertrust-postgres-1 psql -U {{PROJECT}}_app_user -d {{PROJECT}}_dev && npm audit --audit-level=high && yarn lint && echo "SECURITY SUCCESS" || echo "SECURITY FAILED"
```
...
```bash
# Automated RLS check
cat scripts/rls-phase4-final-validation.sql | docker exec -i rendertrust-postgres-1 psql -U {{PROJECT}}_app_user -d {{PROJECT}}_dev
