...
git log --oneline -10 | grep -E "REN-[0-9]+" && echo "COMMIT FORMAT SUCCESS"
...
cat specs/REN-XXX-{feature}-spec.md | grep -A 30 "Pull Request Template"

# Extract logical commits
grep -r "Logical Commits|git commit" specs/REN-XXX-spec.md

# Get demo script for validation
grep -r "Demo Script" specs/REN-XXX-spec.md
```

### 5. Review Documentation

- `CONTRIBUTING.md` - Complete workflow (MANDATORY)
- `specs/REN-XXX-{feature}-spec.md` - Implementation spec with PR template
...
git branch --show-current | grep -E "^REN-[0-9]+-" && echo "✅ Branch name valid"

# 2. Verify commit message format
git log --oneline -1 | grep -E "^[a-z]+(\([a-z]+\))?: .+ \[REN-[0-9]+\]" && echo "✅ Commit format valid"
...
- [ ] Branch name: `REN-{number}-{description}` ✅
- [ ] Commits follow SAFe format: `type(scope): description [REN-XXX]` ✅
...
gh pr create --title "feat(scope): description [REN-XXX]" --body "$(cat <<'EOF'
## 📋 Summary

Implements [feature/fix] as specified in Linear ticket REN-XXX.

**Linear Ticket**: https://linear.app/bybren/issue/REN-XXX
...
3. Select base: `dev` and compare: `REN-{number}-{description}`
...
git commit -m "fix(ci): resolve test failure [REN-XXX]"
...
git add .
git commit -m "refactor(scope): address PR feedback [REN-XXX]"
...
**You do NOT merge** - J. Scott Graham (or designated HITL) is final merge authority.
...
> "PR #XXX for REN-YYY is Ready for HITL Review. All CI green, reviews complete, evidence attached. Awaiting final merge approval from J. Scott Graham."

**Notify J. Scott Graham** and wait for merge.
...
git log --oneline -5 | grep "REN-XXX"

# Linear ticket auto-sync:
# - Tickets referenced in commit messages (e.g., [REN-XXX]) auto-move to Done
...
- SAFe commit format: `type(scope): description [REN-XXX]`
- Branch naming: `REN-{number}-{description}`
...
# 2. Rebase and push
git fetch origin && git rebase origin/dev
git push --force-with-lease origin REN-123-feature

# 3. Create PR
gh pr create --title "feat(feature): implement feature [REN-123]" --web

# 4. Monitor CI
gh pr checks

# 5. Handoff to HITL (RTE does NOT merge)
# Notify J. Scott Graham: "PR #XXX ready for HITL review"
...
git checkout -b REN-999-hotfix-critical-bug
...
gh pr create --base main --title "fix(critical): resolve security issue [REN-999]"

# 4. Handoff to HITL for emergency merge
# Notify J. Scott Graham: "Emergency PR ready - blocks production"
# RTE work ends here - Scott handles merge via GitHub
...
```bash
# Agent A (FE): REN-123-ui-component (depends on REN-124)
# Agent B (BE): REN-124-api-endpoint (must merge first)

# RTE coordinates (but does NOT merge):
# 1. Notify HITL: "REN-124 ready, blocks REN-123"
# Wait for Scott to merge REN-124 via GitHub

# 2. After HITL merges REN-124, RTE rebases REN-123
git checkout REN-123-ui-component
git fetch origin && git rebase origin/dev
git push --force-with-lease

# 3. Notify HITL: "REN-123 ready after REN-124 merged"
# RTE work ends here - Scott handles merge via GitHub
```
...
- **No Merge**: You prepare for merge, HITL (J. Scott Graham) does the merge
...
4. **Handoff Statement**
   > "PR #XXX for REN-YYY is Ready for HITL Review. CI green, reviews complete, evidence attached."
