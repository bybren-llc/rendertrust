...
- User mentions starting work on a ticket (e.g., "I'm starting REN-447")
...
- MUST start with `REN-` followed by ticket number
...
**Required Format**: `type(scope): description [REN-XXX]`
...
Every commit MUST end with `[REN-XXX]` referencing the ticket.

### Examples

```text
feat(harness): create safe-workflow skill [REN-447]
fix(auth): resolve login redirect issue [REN-57]
docs: update API documentation [REN-123]
refactor(db): optimize query performance [REN-234]
chore: upgrade dependencies [REN-337]
```
...
# All commits must follow SAFe format with [REN-XXX]
```
